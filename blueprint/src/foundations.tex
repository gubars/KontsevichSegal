...
\begin{definition}[The Hilbert completion of the rigged triple]
  \label{found:hilbert-completion}
  \notready
  \uses{found:tvs-limits-tensor}
  Completion of the pre-Hilbert space $\check E_\Sigma$, under the reflection-positivity
  pairing, to the Hilbert space $E_\Sigma^{\mathrm{Hilb}}$ sitting in
  $\check E_\Sigma \subset E_\Sigma^{\mathrm{Hilb}} \subset \hat E_\Sigma$ by injective dense
  maps whose composite is the field-theory map $\kappa$ (the assumed \texttt{HilbertFibres}
  and the data of \texttt{InducesUnitaryGH}). Mathlib has Hilbert completion of an
  inner-product space; what is missing is the wiring to the deferred limit spaces
  $\check E$, $\hat E$. \emph{Grind to Mathlib-scale.}
\end{definition}
...
\begin{definition}[The Hodge star on exterior powers]
  ...
  statement. The Hodge star at this generality is not in Mathlib. \emph{Grind to
  Mathlib-scale.}
\end{definition}
...
\begin{definition}[Homogeneous spaces, associated bundles and the $Q_{\mathbb C}$ topology]
  ...
  and~2.6 rest on, and what the two-dimensional polydisc description needs. The same node
  carries the small linear-algebra sub-gap that blocks the Proposition~2.5 proof: the
  Courant--Fischer min--max / eigenvalue-interlacing characterization of the critical angles,
  which is one lemma rather than a body of theory. \emph{Grind to Mathlib-scale.}
\end{definition}

\begin{definition}[Complexified Minkowski space]
  \label{found:minkowski-linear}
  \notready
  Complexified Minkowski space $\mathbb M_{\mathbb C} = \mathbb E \oplus \mathrm i \mathbb E$
  with its $\mathbb C$-bilinear form and the projection onto the real Euclidean subspace
  (the linear-algebra fields of \texttt{MinkowskiComplexGeometry}: \texttt{mC},
  \texttt{bilin}, \texttt{Eucl}, \texttt{projE}). This is finite-dimensional linear algebra,
  largely expressible with existing Mathlib; the totally-real-submanifold half of that class
  is folded into Definition~\ref{found:real-analytic-complexification}. The nearest
  foundation to green. \emph{Grind.}
\end{definition}
