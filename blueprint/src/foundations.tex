...
\begin{definition}[Arbitrary-Lorentzian-geometry parametrization]
  ...
  parametrization only. The closure and openness of the globally hyperbolic subcategory
  (that globally hyperbolic cobordisms are closed under concatenation and form an open
  subcategory) are facts the paper establishes, not abstract bookkeeping; they are proof
  obligations tracked under Definition~\ref{found:lorentzian-causal-geometry}.
\end{definition}
...
\begin{definition}[Lorentzian and causal geometry]
  ...
  hyperbolicity with the closure of globally hyperbolic cobordisms under concatenation and
  the openness of the time-function condition (the \texttt{GhCategory} proof obligations),
  and the geodesic normal form $h_t - \mathrm dt^2$. Lorentzian causal
...
\begin{definition}[Homogeneous spaces, associated bundles and the $Q_{\mathbb C}$ topology]
  ...
  the associated-bundle machinery, and the real Grassmannian. (The topology on
  $Q_{\mathbb C}(V)$ formerly listed here was foundation~F1 and is now BUILT --- see
  Proposition~\ref{prop:contractibility}; this node retains only the
  homogeneous-space and associated-bundle content, and its split into F1 and F2 is
  therefore already half-discharged.) This is what Propositions~2.4
...
\begin{lemma}[Courant--Fischer min--max and eigenvalue interlacing for allowable angles]
  ...
  \lean{ksAngle, ksAngle_antitone, ksAngle_eq_iInf_iSup,
    ksAngle_multiset_eq_angle_cond, ksAngleOn_eq_ksAngleOfForm,
    ksAngle_interlace}
  \leanok
...
\begin{proof}
  \leanok
  \uses{def:allowable, def:restrict-form}
  (Paper, p.~13, for the characterization; the min--max engine is classical.)
  The angle condition of Definition~\ref{def:allowable} yields a rotation
  $e^{-\mathrm i \varphi}$ with $|\varphi| < \pi/2$, taken at the midpoint of
  the arc spanned by the $\arg \lambda_i$, for which
  $\hat g_R = \operatorname{Re}(e^{-\mathrm i \varphi} g)$ is positive definite
  and every $|\arg \lambda_i - \varphi| < \pi/2$. With $\hat g_R$ as inner
  product, $\hat g_I = \operatorname{Im}(e^{-\mathrm i \varphi} g)$ is
  represented by a self-adjoint pencil operator $T$, and
  $\arg g(v) = \arctan\bigl(\langle Tv, v\rangle / \|v\|^2\bigr) + \varphi$
  for $v \neq 0$, principal arguments adding exactly in the open right
  half-plane. The Courant--Fischer min--max for the decreasing eigenvalues
  $\mu_k$ of $T$, proved in both sup-inf and inf-sup forms by dimension count
  against spans of eigenvectors, transports through the monotone continuous
  $\arctan$ and the constant shift: $\theta_k = \arctan \mu_k + \varphi$,
  giving both displayed forms and the decreasing order. Any diagonalizing
  basis witnessing the angle condition is an eigenbasis of $T$ with
  eigenvalues $\tan(\arg \lambda_i - \varphi)$; comparing characteristic
  polynomials identifies the eigenvalue multiset of $T$ with the diagonal
  multiset, so the min--max values are exactly the ordered angles
  $\arg \lambda_i$ of $g$. For $W$ of codimension one, the angles of $g|_W$
  (Definition~\ref{def:restrict-form}) are the same min--max ranged over the
  subspaces contained in $W$: $\theta'_k \leq \theta_k$ because the restricted
  family of subspaces is contained in the ambient family, and
  $\theta_{k+1} \leq \theta'_k$ because the span of the top $k+1$ eigenvectors
  of $T$ meets $W$ in dimension at least $k$, producing a $k$-dimensional
  member of the restricted family on which $\arg g \geq \theta_{k+1}$
  pointwise.
\end{proof}
...
\begin{definition}[Complexified Minkowski space]
  ...
  \lean{WickRotation.MinkowskiLinear, WickRotation.MinkowskiModel.minkowskiLinear,
    WickRotation.MinkowskiModel.negIndex_eq_one}
  \leanok
  ...
  indefinite and nondegenerate, with negative index exactly one, pinning the signature to
  $(d-1,1)$; the forward light cone is exhibited (nonempty, open, proper), its
