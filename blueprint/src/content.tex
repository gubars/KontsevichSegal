...
\begin{theorem}[Equivalence of the two definitions; KS Theorem 2.2]
  ...
  The forward direction (the angle condition implies
  Definition~\ref{def:allowable-hodge}) is formalized and proved. The
  reverse direction requires stating Definition~\ref{def:allowable-hodge}
  for a bare symmetric nondegenerate $\mathbb C$-valued form, with no
  diagonalization assumed, together with the real simultaneous
  diagonalization step of the paper's proof; that type is not yet
  available, so the biconditional is not yet stated in Lean.
\end{theorem}
