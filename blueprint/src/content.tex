...
\begin{proposition}[Contractibility; KS Proposition 2.4]
  ...
  \leanok
  \uses{def:allowable, def:trace-norm-fiber, prop:parametrization}
...
\begin{proof}
  ...
  (Paper, KSTeX 220--222.) Contractibility follows from the fibre-bundle
  description of Proposition~\ref{prop:parametrization}: the fibres
  $\Pi(V,g_0)$ are convex, hence contractible, and the base --- the space of
  positive-definite inner products --- is contractible. The Lean proof is
  therefore gated on foundation~F2, not on F1: it needs the associated-bundle
  decomposition, and it cannot be obtained by applying convexity to
  $Q_{\mathbb C}(V)$ directly, because $Q_{\mathbb C}(V)$ is \emph{not} convex.
  Witness at $d = 1$: for $V = \mathbb R$ an allowable metric is
  $g(v,w) = \lambda vw$ with $\lambda \in \mathbb C \setminus (-\infty, 0]$, and
  the segment from $-1 + \varepsilon \mathrm i$ to $-1 - \varepsilon \mathrm i$
  passes through $-1$. (At $d = 1$ the set is star-convex about $1$, so that
  special case is separately provable; the general claim is not, and KS prove it
  via the bundle.) The Lean statement therefore carries a tracked \texttt{sorry}.
\end{proof}
