...
| | **BUILT 2026-06-26 (scaffold-green):** completion machinery discharged in `HilbertCompletion.lean` (`structure HilbertCompletion` + model `hilbertCompletion` over `UniformSpace.Completion`; Mathlib ships `inner_coe`/`innerProductSpace`). The `E^Hilb ⊂ Ê` wiring + `κ`-factorisation stay deferred to F2 (`found:tvs-limits-tensor`), so the node KEEPS that `\uses` edge. | | | |
